...
class Yolov8Visualizer(Node):
    ...
    def __init__(self):
        ...
        default_detections_classes_file_path = os.path.join(
            get_package_share_directory("isaac_ros_yolov8"),
            "config",
            "sauvc_detection_classes.yaml",
        )
        self.declare_parameter(
            "detections_classes_file_path", default_detections_classes_file_path
        )
        detections_classes_file_path = (
            self.get_parameter("detections_classes_file_path")
            .get_parameter_value()
            .string_value
        )
        with open(detections_classes_file_path, "r") as f:
            yaml_params = yaml.safe_load(f)
            self.names = yaml_params["detection_classes"]
        ...
        self.time_synchronizer = message_filters.TimeSynchronizer(
            [self._detections_subscription, self._image_subscription], self.QUEUE_SIZE
        )
    ...
    def detections_callback(self, detections_msg, img_msg):
        ...
        for detection in detections_msg.detections:
            ...
            label = self.names[int(detection.results[0].hypothesis.class_id)]
        ...
        processed_img = self._bridge.cv2_to_imgmsg(cv2_img, encoding=img_msg.encoding)
